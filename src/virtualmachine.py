...
class VirtualMachine():
    ...
    def evalValue(self):
        if not isinstance(self.expr, (tokens.symbol.Symbol, tokens.number.Number, Lst, tokens.string.String, tokens.literal.Literal)):
            self.vals = self.expr
    ...
    def evalContinuation(self):
        ...
        if k == ContinuationType.cEmpty:
            ...
            self.env = env
            self.continuation = Continuation(
                ContinuationType.cMapValueOfCons, first, k)
            self.counter = self.evalMapValue
            return
