...
def get_libs():
    ...
    for dirs in os.listdir(std_path):
        if os.path.isdir(os.path.join(std_path,
                                      dirs)) and not dirs.startswith("__"):
            std_libs.update(
                {os.path.basename(dirs): os.path.join(std_path, dirs)})
    ...
    for root, dirs, files in os.walk(path):
        for i in files:
            if i.endswith(".py") and not i.endswith("__init__.py"):
                return_list.append(
                    os.path.join(root, i).replace(std_path + "/", ""))
            elif i.endswith(".pyl"):
